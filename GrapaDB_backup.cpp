...
#define DEBUG_PTRTOREC 1
#ifdef DEBUG_PTRTOREC
#include <stdio.h>
#endif
...
GrapaError GrapaDB::CreateRecord(GrapaDBTable& pTable, GrapaCursor& pCursor)
{
...
#ifdef DEBUG_PTRTOREC
	printf("DEBUG CreateRecord: ENTERED function\n");
	fflush(stdout);
#endif
...
#ifdef DEBUG_PTRTOREC
	printf("DEBUG CreateRecord: pTable.mDictField.mTreeType=%d\n", pTable.mDictField.mTreeType);
	fflush(stdout);
#endif
	...
	switch (pTable.mDictField.mTreeType)
	{
		case SU64_TREE:
#ifdef DEBUG_PTRTOREC
			printf("DEBUG CreateRecord: In SU64_TREE case\n");
			fflush(stdout);
#endif
		...
		case GROUP_TREE:
#ifdef DEBUG_PTRTOREC
			printf("DEBUG CreateRecord: In GROUP_TREE case\n");
			fflush(stdout);
#endif
		...
		case RTABLE_TREE:
#ifdef DEBUG_PTRTOREC
			printf("DEBUG CreateRecord: In RTABLE_TREE case\n");
			fflush(stdout);
#endif
			{
				...
				if (!err && indexCursor.mKey==0)
					err = Next(indexCursor);
#ifdef DEBUG_PTRTOREC
				printf("DEBUG CreateRecord: >>> ENTERED for key=%llu <<<\n", pCursor.mKey);
				fflush(stdout);
#endif
				while (!err)
				{
#ifdef DEBUG_PTRTOREC
					printf("DEBUG CreateRecord: Before Insert for key=%llu\n", pCursor.mKey);
					for (u64 leaf = 0; leaf < 4; ++leaf) {
						GrapaCursor ptrCursor;
						ptrCursor.Set(84, RPTR_ITEM, leaf + 1);
						GrapaError err2 = Search(ptrCursor);
						printf("DEBUG: Node 84, leaf %llu (key=%llu): err=%lld, mValue=%llu, mNodeRef=%llu, mNodeIndex=%d\n",
							leaf, ptrCursor.mKey, (long long)err2, ptrCursor.mValue, ptrCursor.mNodeRef, ptrCursor.mNodeIndex);
					}
#endif
					tableCursor.Set(indexCursor.mValue,RPTR_ITEM,pCursor.mKey);
					err = Insert(tableCursor);
#ifdef DEBUG_PTRTOREC
					printf("DEBUG CreateRecord: After Insert for key=%llu\n", pCursor.mKey);
					for (u64 leaf = 0; leaf < 4; ++leaf) {
						GrapaCursor ptrCursor;
						ptrCursor.Set(84, RPTR_ITEM, leaf + 1);
						GrapaError err2 = Search(ptrCursor);
		...
		case CTABLE_TREE:
#ifdef DEBUG_PTRTOREC
			printf("DEBUG CreateRecord: In CTABLE_TREE case\n");
			fflush(stdout);
#endif
			{
		...
		default:
#ifdef DEBUG_PTRTOREC
			printf("DEBUG CreateRecord: In DEFAULT case\n");
			fflush(stdout);
#endif
			return(-1);
			break;
...
GrapaError GrapaDB::SetRecordField(GrapaCursor& cursor, GrapaDBFieldValueArray& pFieldList)
{
...
#ifdef DEBUG_PTRTOREC
	printf("DEBUG SetRecordField: PtrToRec input cursor: type=%d key=%llu value=%llu\n", cursor.mValueType, cursor.mKey, cursor.mValue);
	printf("DEBUG SetRecordField: PtrToRec output recCursor: type=%d key=%llu value=%llu err=%lld\n", recCursor.mValueType, recCursor.mKey, recCursor.mValue, (long long)err);
#endif
	if (err) return(err);
	...
	{
#ifdef DEBUG_PTRTOREC
		printf("DEBUG SetRecordField: IndexCursor before update: type=%d key=%llu value=%llu\n", indexCursor.mValueType, indexCursor.mKey, indexCursor.mValue);
#endif
...
#ifdef DEBUG_PTRTOREC
						printf("DEBUG SetRecordField: Deleting GROUP_TREE index: tableCursor type=%d key=%llu value=%llu\n", tableCursor.mValueType, tableCursor.mKey, tableCursor.mValue);
#endif
						err = Delete(tableCursor);
						break;
					case RTABLE_TREE:
...
#ifdef DEBUG_PTRTOREC
						printf("DEBUG SetRecordField: Deleting RTABLE_TREE index: tableCursor type=%d key=%llu value=%llu\n", tableCursor.mValueType, tableCursor.mKey, tableCursor.mValue);
						printf("DEBUG: Before Delete for key=%llu\n", tableCursor.mKey);
						for (u64 leaf = 0; leaf < 4; ++leaf) {
							GrapaCursor ptrCursor;
							ptrCursor.Set(84, RPTR_ITEM, leaf + 1);
							GrapaError err = Search(ptrCursor);
							printf("DEBUG: Node 84, leaf %llu (key=%llu): err=%lld, mValue=%llu, mNodeRef=%llu, mNodeIndex=%d\n",
								leaf, ptrCursor.mKey, (long long)err, ptrCursor.mValue, ptrCursor.mNodeRef, ptrCursor.mNodeIndex);
						}
#endif
						err = Delete(tableCursor);
#ifdef DEBUG_PTRTOREC
						printf("DEBUG: After Delete for key=%llu\n", tableCursor.mKey);
						for (u64 leaf = 0; leaf < 4; ++leaf) {
							GrapaCursor ptrCursor;
							ptrCursor.Set(84, RPTR_ITEM, leaf + 1);
							GrapaError err = Search(ptrCursor);
...
#endif
						DebugPrintIndexPointerAndRecord(indexCursor.mValue, 1);
						DebugPrintIndexPointerAndRecord(indexCursor.mValue, 2);
						DebugPrintIndexPointerAndRecord(indexCursor.mValue, 3);
						DebugPrintIndexPointerAndRecord(indexCursor.mValue, 4);
						break;
					case CTABLE_TREE:
						tableCursor.Set(indexCursor.mValue,CPTR_ITEM,recCursor.mKey);
#ifdef DEBUG_PTRTOREC
						printf("DEBUG SetRecordField: Deleting CTABLE_TREE index: tableCursor type=%d key=%llu value=%llu\n", tableCursor.mValueType, tableCursor.mKey, tableCursor.mValue);
#endif
						err = Delete(tableCursor);
						break;
				}
#ifdef DEBUG_PTRTOREC
				printf("DEBUG SetRecordField: After Delete, err=%lld\n", (long long)err);
#endif
				break;
			}
		}
...
#ifdef DEBUG_PTRTOREC
	printf("DEBUG SetRecordField: Starting index update\n");
#endif
	...
	while (!err)
	{
		...
		{
#ifdef DEBUG_PTRTOREC
			printf("DEBUG SetRecordField: index update itteration=%llu\n",i);
#endif
			dbFieldValue = pFieldList.GetFieldAt(i);
			if (IndexHasField(indexCursor, dbFieldValue->mId))
			{
				...
				case RTABLE_TREE:
					tableCursor.Set(indexCursor.mValue, RPTR_ITEM, recCursor.mKey);
#ifdef DEBUG_PTRTOREC
					printf("DEBUG SetRecordField: In index update loop BEFORE insert - recCursor.mKey=%llu, tableCursor.mKey=%llu\n", recCursor.mKey, tableCursor.mKey);
					printf("DEBUG SetRecordField: tableCursor: mTreeRef=%llu, mValueType=%d, mKey=%llu, mValue=%llu, mNodeRef=%llu, mNodeIndex=%d, mLength=%llu\n",
						tableCursor.mTreeRef, tableCursor.mValueType, tableCursor.mKey, tableCursor.mValue, tableCursor.mNodeRef, tableCursor.mNodeIndex, tableCursor.mLength);
					printf("DEBUG SetRecordField: recCursor: mTreeRef=%llu, mValueType=%d, mKey=%llu, mValue=%llu, mNodeRef=%llu, mNodeIndex=%d, mLength=%llu\n",
						recCursor.mTreeRef, recCursor.mValueType, recCursor.mKey, recCursor.mValue, recCursor.mNodeRef, recCursor.mNodeIndex, recCursor.mLength);

					printf("DEBUG: Before Insert for key=%llu\n", tableCursor.mKey);
					for (u64 leaf = 0; leaf < 4; ++leaf) {
						GrapaCursor ptrCursor;
						ptrCursor.Set(84, RPTR_ITEM, leaf + 1); // assuming keys are 1-based
						GrapaError err = Search(ptrCursor);
						printf("DEBUG: Node 84, leaf %llu (key=%llu): mValue=%llu, mNodeRef=%llu, mNodeIndex=%d\n",
							leaf, ptrCursor.mKey, ptrCursor.mValue, ptrCursor.mNodeRef, ptrCursor.mNodeIndex);
					}
#endif
					err = Insert(tableCursor);
#ifdef DEBUG_PTRTOREC
					printf("DEBUG: After Insert for key=%llu\n", tableCursor.mKey);
					for (u64 leaf = 0; leaf < 4; ++leaf) {
						GrapaCursor ptrCursor;
						ptrCursor.Set(84, RPTR_ITEM, leaf + 1);
						GrapaError err = Search(ptrCursor);
					...
					printf("DEBUG SetRecordField: In index update loop AFTER insert - recCursor.mKey=%llu, tableCursor.mKey=%llu, err=%lld\n", recCursor.mKey, tableCursor.mKey, (long long)err);
					DebugPrintAllIndexPointers(indexCursor.mValue);
					DebugPrintIndexPointerAndRecord(indexCursor.mValue, 1);
					DebugPrintIndexPointerAndRecord(indexCursor.mValue, 2);
					DebugPrintIndexPointerAndRecord(indexCursor.mValue, 3);
					DebugPrintIndexPointerAndRecord(indexCursor.mValue, 4);
...
GrapaError GrapaDB::PtrToRec(GrapaCursor& ptrCursor, GrapaCursor& recCursor)
{
	...
	u8 storeType;
#ifdef DEBUG_PTRTOREC
	printf("DEBUG PtrToRec: input ptrCursor: type=%d key=%llu value=%llu\n", ptrCursor.mValueType, ptrCursor.mKey, ptrCursor.mValue);
#endif
	recCursor = ptrCursor;
	switch(ptrCursor.mValueType)
	{
		...
		case CREC_ITEM:
#ifdef DEBUG_PTRTOREC
			printf("DEBUG PtrToRec: Already a record type, returning 0\n");
#endif
			return(0);
		case GPTR_ITEM:
		case RPTR_ITEM:
		...
		default:
#ifdef DEBUG_PTRTOREC
			printf("DEBUG PtrToRec: Invalid type, returning -1\n");
#endif
			return(-1);
			break;
	}
...
#ifdef DEBUG_PTRTOREC
	printf("DEBUG PtrToRec: After GetTreeStore: tableRef=%llu storeType=%u err=%lld\n", tableRef, (unsigned)storeType, (long long)err);
#endif
	if (err) return(err);
	switch(ptrCursor.mValueType)
	{
...
#ifdef DEBUG_PTRTOREC
			printf("DEBUG PtrToRec: GPTR_ITEM -> GREC_ITEM: recCursor type=%d key=%llu value=%llu\n", recCursor.mValueType, recCursor.mKey, recCursor.mValue);
#endif
			break;
		case RPTR_ITEM:
...
#ifdef DEBUG_PTRTOREC
			printf("DEBUG PtrToRec: RPTR_ITEM -> RREC_ITEM: recCursor type=%d key=%llu value=%llu\n", recCursor.mValueType, recCursor.mKey, recCursor.mValue);
#endif
			break;
		case CPTR_ITEM:
...
#ifdef DEBUG_PTRTOREC
			printf("DEBUG PtrToRec: CPTR_ITEM -> CREC_ITEM: recCursor type=%d key=%llu value=%llu\n", recCursor.mValueType, recCursor.mKey, recCursor.mValue);
#endif
			break;
	}
	GrapaError searchErr = Search(recCursor);
#ifdef DEBUG_PTRTOREC
	printf("DEBUG PtrToRec: After Search: recCursor type=%d key=%llu value=%llu err=%lld\n", recCursor.mValueType, recCursor.mKey, recCursor.mValue, (long long)searchErr);
#endif
	return searchErr;
}
...
void GrapaDB::DebugPrintAllIndexPointers(u64 tableRef) {
#ifdef COMMENTOUTFORNOW
	u64 indexRef = 0;
    GrapaCursor indexCursor, ptrCursor;
    GrapaError err;
    indexCursor.Set(tableRef);
    err = GetTreeIndex(indexCursor, indexRef);
    if (err || indexRef == 0) {
    ...
    printf("DEBUG: Full index tree for tableRef=%llu\n", tableRef);
    for (u64 k = 1; k <= 4; ++k) {
        ptrCursor.Set(indexRef, RPTR_ITEM, k);
        err = Search(ptrCursor);
        u64 weight = 0;
        DumpGetItemWeight(ptrCursor, weight);
        printf("DEBUG: RPTR_ITEM key=%llu: valueType=%d key=%llu value=%llu node=(%llu,%d) weight=%llu\n",
            k, ptrCursor.mValueType, ptrCursor.mKey, ptrCursor.mValue, ptrCursor.mNodeRef, ptrCursor.mNodeIndex, weight);
    }
#endif
}
#endif

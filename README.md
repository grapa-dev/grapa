...
There will be an installer for each of the platforms...but for now, drag and drop of the binary works. On Linux, place in "/usr/bin" (or somewhere accessable). On Windows, place in a location that is in the PATH, or update PATH to the location the application is copied to. On Mac, copy to "/usr/local/bin" - first time running the tool will take about 20 seconds as the OS scans the executable. 

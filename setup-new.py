...
if sys.platform.startswith('linux'):
    ...
    if is_arm:
        from_os = 'linux-arm64'
    # FLTK libraries require X11 system libraries, and we need bundled OpenSSL 1.1.1w for SRP support
    if is_arm:
        extra_link_args = ['-std=c++17','-O3','-pthread','-ldl','-lm','-lX11','-lXfixes','-lXft','-lXext','-lXrender','-lXinerama','-lfontconfig','-lXcursor','-Wl,-rpath,$ORIGIN']
    else:
        extra_link_args = ['-std=c++17','-O3','-pthread','-ldl','-lm','-lX11','-lXfixes','-lXft','-lXext','-lXrender','-lXinerama','-lfontconfig','-lXcursor','-Wl,-rpath,$ORIGIN']
...
class CopySharedLibrary(Command):
    ...
    def run(self) -> None:
        ...
        if sys.platform.startswith('linux') or sys.platform.startswith('darwin'):
            ...
            # Copy OpenSSL 1.1.1w libraries for $ORIGIN runtime loading
            for file_name in os.listdir(os.path.join(self.build_dir, 'openssl-lib/'+from_os+'/runtime')):
                self.copy_file(os.path.join(os.path.join(self.build_dir, 'openssl-lib/'+from_os+'/runtime'),file_name), os.path.join(lib_target_path, file_name))
        ...
        if not sys.platform.startswith('win32'):
            ...
            try:
                if sys.platform.startswith('darwin'):
                    ...
                    # Linux: always compile utf8proc from source for architecture compatibility
                    print("Compiling utf8proc from source for Linux...")
                    # Remove any existing utf8proc.o that might be for wrong architecture
                    if os.path.exists('utf8proc.o'):
                        os.remove('utf8proc.o')
                    # Compile utf8proc for current architecture
                    subprocess.run([
                    ...
                    if os.path.exists('utf8proc.o'):
                        ...
                        print("Warning: Failed to compile utf8proc")
...
def pick_library_dirs():
    ...
    if my_system == 'Linux':
        if is_arm:
            return ["source", "source/grapa-lib/linux-arm64", "source/blst-lib/linux-arm64", "source/fl-lib/linux-arm64", "source/pcre2-lib/linux-arm64"]  # Static libraries only, OpenSSL handled via $ORIGIN
        else:
            return ["source", "source/grapa-lib/linux-amd64", "source/blst-lib/linux-amd64", "source/fl-lib/linux-amd64", "source/pcre2-lib/linux-amd64"]  # Static libraries only, OpenSSL handled via $ORIGIN
    if my_system == 'Darwin':
        if is_arm:
...
def pick_libraries():
    my_system = platform.system()
    if my_system == 'Linux':
        return ['grapa']  # Only grapa library, everything else handled via $ORIGIN like setup_0.0.42.py
    if my_system == 'Darwin':
...
# Prepare include directories - prioritize OpenSSL 1.1.1w headers over system headers
include_dirs = ["source/openssl","source","source/utf8proc","source/pybind11/include","source/X11","source/FL","openssl-1.1.1w/include"]

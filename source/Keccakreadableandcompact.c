...
/**
  ...
  */
void xKeccak(unsigned int rate, unsigned int capacity, const unsigned char *input, unsigned long long int inputByteLen, unsigned char delimitedSuffix, unsigned char *output, unsigned long long int outputByteLen);
...
{
    xKeccak(1344, 256, input, inputByteLen, 0x1F, output, outputByteLen);
}
...
{
    xKeccak(1088, 512, input, inputByteLen, 0x1F, output, outputByteLen);
}
...
{
    xKeccak(1152, 448, input, inputByteLen, 0x06, output, 28);
}
...
{
    xKeccak(1088, 512, input, inputByteLen, 0x06, output, 32);
}
...
{
    xKeccak(832, 768, input, inputByteLen, 0x06, output, 48);
}
...
{
    xKeccak(576, 1024, input, inputByteLen, 0x06, output, 64);
}
 ...
 */
void xKeccakF1600_StatePermute(void *state)
{
...
void xKeccak(unsigned int rate, unsigned int capacity, const unsigned char *input, unsigned long long int inputByteLen, unsigned char delimitedSuffix, unsigned char *output, unsigned long long int outputByteLen)
{
    ...
    while(inputByteLen > 0) {
        ...
        if (blockSize == rateInBytes) {
            xKeccakF1600_StatePermute(state);
            blockSize = 0;
        }
    }

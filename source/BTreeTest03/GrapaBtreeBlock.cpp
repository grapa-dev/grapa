...
GrapaError GrapaBlockNodeHeader::Write(GrapaFile *pFile, u64 blockPos)
{
	if (!pFile) return(-1);
	if (blockPos == 92) {
		printf("[DEBUG][BTree][BlockAccess] blockPos=%llu operation=WRITE\n", blockPos);
	}
	BigEndian();
	GrapaError err = pFile->Write(blockPos, GrapaBlock::BLOCKSIZE, 0, GetSize(), &blockType);
	BigEndian();
	// After writing to block 92 in GrapaBlockNodeHeader::Write and GrapaBlockNodeLeaf::Write:
	if (blockPos == 92) {
		printf("[DEBUG][BTree][BlockAccess] blockPos=%llu operation=WRITE\n", blockPos);
		// Dump all leafs in block 92
		GrapaBlockNodeHeader nodeHeader;
		if (nodeHeader.Read(pFile, 92) == 0) {
			printf("[DEBUG][BTree][Block92Dump] leafCount=%d\n", nodeHeader.leafCount);
			for (int i = 0; i < nodeHeader.leafCount; ++i) {
				GrapaBlockNodeLeaf leaf;
				if (leaf.Read(pFile, 92 + 1 + i) == 0) {
					printf("  [DEBUG][BTree][Block92Dump] leaf[%d]: key=%llu value=%llu child=%llu\n", i, leaf.key, leaf.value, leaf.child);
				}
			}
		}
	}
...
GrapaError GrapaBlockNodeHeader::Read(GrapaFile *pFile, u64 blockPos)
{
	if (!pFile) return(-1);
	if (blockPos == 92) {
		printf("[DEBUG][BTree][BlockAccess] blockPos=%llu operation=READ\n", blockPos);
	}
...
{
	GrapaError err = Write(pFile, GetOffset(blockPos, indexPos));
	// After writing to block 92 in GrapaBlockNodeHeader::Write and GrapaBlockNodeLeaf::Write:
	if (blockPos == 92) {
		printf("[DEBUG][BTree][BlockAccess] blockPos=%llu operation=WRITE\n", blockPos);
		// Dump all leafs in block 92

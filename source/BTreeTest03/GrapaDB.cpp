	...
	while (!err)
	{
		for (i = 0; i < fieldCount; i++)
		{
			...
			if (IndexHasField(indexCursor, dbFieldValue->mId))
			{
				...
				// RTABLE_TREE case for SetRecordField with defensive delete, single insert, and debug output
				case RTABLE_TREE:
					// Set up the index entry for this key
					tableCursor.Set(indexCursor.mValue, RPTR_ITEM, recCursor.mKey, recCursor.mValue);

					// Defensive: delete any stale index entry
					delErr = Delete(tableCursor);

					// Debug output: show the delete and the flat index state
					if (tableCursor.mValueType == RPTR_ITEM) {
						printf("[DEBUG][SetRecordField][Delete][RPTR_ITEM] key=%llu value=%llu treeRef=%llu err=%lld\n",
							tableCursor.mKey, tableCursor.mValue, tableCursor.mTreeRef, delErr);
...
GrapaError GrapaDB::GetRecordField(GrapaCursor& cursor, GrapaDBField& field, GrapaBYTE& buffer)
{
    ...
    indexTree = tree.indexTree;
    storeTree = tree.storeTree;
    printf("[DEBUG][GrapaDB][GetRecordField-Field] treeType=%d indexTree=%llu storeTree=%llu dataRef=%llu\n", (int)cursor.mTreeType, indexTree, storeTree, dataRef);

    err = PtrToRec(cursor,recCursor);
    if (err) {
        printf("[DEBUG][GrapaDB][GetRecordField-Field] PtrToRec error: %lld\n", (long long)err);
        return(err);
    }
    printf("[DEBUG][GrapaDB][GetRecordField-Field] recCursor.key=%llu recCursor.value=%llu recCursor.treeType=%d\n", recCursor.mKey, recCursor.mValue, (int)recCursor.mTreeType);
	...
	case RTABLE_TREE:
        printf("[DEBUG][GrapaDB][GetRecordField-Field] RTABLE_TREE case: dataRef=%llu\n", dataRef);
		switch (field.mStore)
		{
		case GrapaDBField::STORE_FIX:
            printf("[DEBUG][GrapaDB][GetRecordField-Field] STORE_FIX case: dictSize=%llu dictOffset=%llu\n", field.mDictSize, field.mDictOffset);
			if (field.mDictSize == 1)
			{
                printf("[DEBUG][GrapaDB][GetRecordField-Field] STORE_FIX dictSize=1: calling GetDataValue\n");
				err = GetDataValue(dataRef, field.mDictOffset, 1, h, &returnSize);
                if (err) {
                    printf("[DEBUG][GrapaDB][GetRecordField-Field] GetDataValue error: %lld\n", (long long)err);
                    return(err);
                }
                printf("[DEBUG][GrapaDB][GetRecordField-Field] GetDataValue result: h[0]=%d returnSize=%llu\n", h[0], returnSize);
				if ((h[0] & 0x80) == 0)
					buffer.SetSize(0);
				else
				{
		...
		case GrapaDBField::STORE_VAR:
		case GrapaDBField::STORE_PAR:
            printf("[DEBUG][GrapaDB][GetRecordField-Field] STORE_VAR/PAR case: calling GetDataValue for dataPtr\n");
			err = GetDataValue(dataRef, field.mDictOffset, sizeof(dataPtr), &dataPtr, &returnSize);
            if (err) {
                printf("[DEBUG][GrapaDB][GetRecordField-Field] GetDataValue for dataPtr error: %lld\n", (long long)err);
                return(err);
            }
            printf("[DEBUG][GrapaDB][GetRecordField-Field] GetDataValue result: dataPtr=%llu returnSize=%llu\n", dataPtr, returnSize);
			if (dataPtr)
			{
				dataPtr = BE_S64(dataPtr);
                printf("[DEBUG][GrapaDB][GetRecordField-Field] After BE_S64: dataPtr=%llu\n", dataPtr);
				err = GetDataSize(dataPtr, growBlockSize, dataSize, dataLength, compressType);
                if (err) {
                    printf("[DEBUG][GrapaDB][GetRecordField-Field] GetDataSize error: %lld\n", (long long)err);
                    return(err);
                }
                printf("[DEBUG][GrapaDB][GetRecordField-Field] GetDataSize result: dataSize=%llu dataLength=%llu\n", dataSize, dataLength);
				buffer.SetLength(dataLength, false);
				err = GetDataValue(dataPtr, 0, dataLength, buffer.GetPtr(), &returnSize);
                if (err) {
                    printf("[DEBUG][GrapaDB][GetRecordField-Field] GetDataValue for buffer error: %lld\n", (long long)err);
                    return(err);
                }
                printf("[DEBUG][GrapaDB][GetRecordField-Field] GetDataValue for buffer result: returnSize=%llu\n", returnSize);
			}
			else
			{
                printf("[DEBUG][GrapaDB][GetRecordField-Field] dataPtr is 0, setting buffer size to 0\n");
				buffer.SetSize(0);
			}
			if (isRaw)
			{
            ...
            printf("[DEBUG][GrapaDB][GetRecordField-Field] STORE_VAR/PAR case completed successfully\n");
			break;
		}
		break;
...
{
    printf("[DEBUG][GrapaDB][GetDataTypeRecord] input: tableRef=%llu\n", tableRef);
    GrapaError err=0;
    GrapaCursor tableCursor;
    u64 indexTree=0;
    tableDT = 0;
    ...
    err = GetTreeIndex(tableCursor,indexTree);
    if (err) {
        printf("[DEBUG][GrapaDB][GetDataTypeRecord] GetTreeIndex error: %lld\n", (long long)err);
        return(err);
    }
    printf("[DEBUG][GrapaDB][GetDataTypeRecord] indexTree=%llu\n", indexTree);
    tableCursor.Set(indexTree);
    err = Search(tableCursor);
    if (err) {
        printf("[DEBUG][GrapaDB][GetDataTypeRecord] Search error: %lld\n", (long long)err);
        return(err);
    }
    tableDT = tableCursor.mValue;
    printf("[DEBUG][GrapaDB][GetDataTypeRecord] output: tableDT=%llu\n", tableDT);
    return(0);
}
...
GrapaError GrapaDB::PtrToRec(GrapaCursor& ptrCursor, GrapaCursor& recCursor)
{
    ...
    switch(ptrCursor.mValueType)
    {
        ...
        case RPTR_ITEM:
            ...
            recCursor.mValue = ptrCursor.mValue;  // Set the record data pointer
            break;
        case CPTR_ITEM:
...
{
    printf("[DEBUG][GrapaDBField][Read] input: fieldRef=%llu\n", fieldRef);
    GrapaError err = pDb->GetDataValue(fieldRef,0,GetSize(),GetPtr());
    if (err) {
        printf("[DEBUG][GrapaDBField][Read] GetDataValue error: %lld\n", (long long)err);
        return(err);
    }
    BigEndian();
    return(err);
...
{
    printf("[DEBUG][GrapaDBField][Get] input: tableRef=%llu fieldId=%llu\n", tableRef, fieldId);
    GrapaError err=0;
    ...
    if (err) {
        printf("[DEBUG][GrapaDBField][Get] GetDataTypeRecord error: %lld\n", (long long)err);
        return(err);
    }
    printf("[DEBUG][GrapaDBField][Get] tableDT=%llu\n", tableDT);

    dataTypeCursor.Set(tableDT,GrapaDB::SU64_ITEM,fieldId,0);
    err = pDb->Search(dataTypeCursor);
    if (err) {
        printf("[DEBUG][GrapaDBField][Get] Search error: %lld\n", (long long)err);
        return(err);
    }
    printf("[DEBUG][GrapaDBField][Get] dataTypeCursor.mValue=%llu\n", dataTypeCursor.mValue);

    err = Read(pDb,dataTypeCursor.mValue);
    if (err) {
        printf("[DEBUG][GrapaDBField][Get] Read error: %lld\n", (long long)err);
        return(err);
    }

...
GrapaRuleEvent* GrapaScriptState::AddRuleOperation(GrapaRuleQueue* pRuleEvent, const char* pName, const char* pValue, s64 a, s64 b, s64 c, s64 d)
{
	...
	GrapaRuleEvent *v;
	v = new GrapaRuleEvent(GrapaTokenType::GOBJ, 0, "", "");
...
GrapaRuleEvent* GrapaScriptExec::PlanRule(GrapaNames* pNameSpace, GrapaRuleEvent* pTokenEvent, GrapaRuleEvent* pRuleEvent, GrapaRuleQueue* pOperationQueue, bool& pMatched, GrapaRuleEvent* pLastNext, bool& pAddLastNext, s8 pLeftRecursion, GrapaKeyValue& pCache, bool pTouched)
{
	...
	while (operation)
	{
		...
		while (parameter && nextEvent)
		{
			...
			if (parameter->mValue.mToken == GrapaTokenType::REF || parameter->mValue.mToken == GrapaTokenType::GOBJ)
			{
				...
				if (parameter->mValue.mToken == GrapaTokenType::REF)
				{
					...
					if (p && p->mValue.mToken == GrapaTokenType::GOBJ)
					{
				...
				else if (dList)
				{
					...
					else if (e->mValue.mToken == GrapaTokenType::LIST || e->mValue.mToken == GrapaTokenType::TUPLE || e->mValue.mToken == GrapaTokenType::GOBJ || e->mValue.mToken == GrapaTokenType::ERR || e->mValue.mToken == GrapaTokenType::XML || e->mValue.mToken == GrapaTokenType::EL || e->mValue.mToken == GrapaTokenType::TAG || e->mValue.mToken == GrapaTokenType::OP || e->mValue.mToken == GrapaTokenType::CODE || e->mValue.mToken == GrapaTokenType::ERR)
					{
			...
			else if (parameter->mValue.mToken == GrapaTokenType::RULEREF)
			{
				...
				else if (parameter->vQueue == NULL || (parameter->vQueue->mCount == 0 || (parameter->vQueue->mCount && !parameter->mName.mLength)))
				{
					...
					if (start)
					{
						GrapaRuleEvent* e = new GrapaRuleEvent(GrapaTokenType::LIST, 0, "");
						e->vQueue = new GrapaRuleQueue();
...
GrapaRuleEvent* GrapaScriptExec::ProcessPlan(GrapaNames* pNameSpace, GrapaRuleEvent* pOperation, GrapaRuleEvent* pParam, u64 pCount)
{
	...
	else if (pOperation->mValue.mToken == GrapaTokenType::OP)
	{
		...
		if (pOperation->vQueue && pOperation->vQueue->mCount)
		{
			...
			case 2:
				...
				if (inputItem && inputItem->mValue.mToken == GrapaTokenType::GOBJ && !inputItem->mNull)
				{
					libName = &gSystem->mLib;
					libParam = (GrapaRuleEvent*)pOperation->vQueue->Head();
				}
	...
	else if (!pOperation->mNull)
	{
		...
		if (!pOperation->mControlFlow && pOperation->mValue.mToken == GrapaTokenType::PTR && pOperation->vRulePointer)
			pOperation->mControlFlow = pOperation->vRulePointer->mControlFlow;
...
//		if (item->mValue.mToken == GrapaTokenType::GOBJ)
//		{
//			if (item->vQueue)
//			{
...
//			if (r1)
//			{
//				if (r1->mValue.mToken == GrapaTokenType::GOBJ)
//				{
...
void GrapaScriptExec::EchoValue(GrapaSystemSend* pSend, GrapaRuleEvent* pTokenEvent, bool elMode, bool strquote, bool fromlist)
{
	...
	case GrapaTokenType::TABLE:
		...
		break;
	case GrapaTokenType::PTR:
		if (pTokenEvent->vRulePointer && (pTokenEvent->vRulePointer->mValue.mToken == GrapaTokenType::ERR || pTokenEvent->vRulePointer->mValue.mToken == GrapaTokenType::LIST || pTokenEvent->vRulePointer->mValue.mToken == GrapaTokenType::TUPLE || pTokenEvent->vRulePointer->mValue.mToken == GrapaTokenType::GOBJ || pTokenEvent->vRulePointer->mValue.mToken == GrapaTokenType::XML || pTokenEvent->vRulePointer->mValue.mToken == GrapaTokenType::EL || pTokenEvent->vRulePointer->mValue.mToken == GrapaTokenType::TAG || pTokenEvent->vRulePointer->mValue.mToken == GrapaTokenType::OP || pTokenEvent->vRulePointer->mValue.mToken == GrapaTokenType::OBJ || pTokenEvent->vRulePointer->mValue.mToken == GrapaTokenType::ERR))
...
void GrapaScriptExec::EchoTokenValue(GrapaSystemSend* pSend, GrapaRuleEvent* pEvent, bool elMode, bool pretty)
{
	...
	case GrapaTokenType::GOBJ: pSend->Send(this, vScriptState->vRuleVariables, "$GOBJ "); break;
	...
	else
	{
		if (scanOps->vRuleParent && scanOps->vRuleParent->vRuleLambda)
		{
			...
			if (result2->vQueue)
			{
				...
				result3 = new GrapaRuleEvent(GrapaTokenType::GOBJ, 0, "", "");
		...
		if (result2 && result2->vQueue)
		{
			...
			case 1:
				...
				break;
			case 2:
				if (ptail && ptail->mValue.mToken == GrapaTokenType::GOBJ && !ptail->mNull)
				{
					code = &gSystem->mLib;
					param = (GrapaRuleEvent*)result2->vQueue->Head();
				}
...
GrapaRuleEvent *GrapaScriptExec::Plan(GrapaNames* pNameSpace, GrapaCHAR& pInput, GrapaRuleEvent* rulexx, u64 pRuleId, GrapaCHAR pProfile)
{
	...
	GrapaRuleEvent* codeResult = new GrapaRuleEvent(GrapaTokenType::LIST, 0, "", "");
	...
	if (codeResult)
	{
		/// need a unique LIST type for this....otherwise an data array of 1 item becomes that 1 item.
		// For now, see if only doing this once is sufficient
		if ((codeResult->mValue.mToken == GrapaTokenType::LIST || codeResult->mValue.mToken == GrapaTokenType::TUPLE) && codeResult->vQueue)
		{
...
GrapaScriptExecStateDebug::GrapaScriptExecStateDebug() 
{
	...
	const char* sessionDebug = getenv("GRAPA_SESSION_DEBUG_MODE");
...
void GrapaScriptExecStateDebug::DebugPrint(GrapaScriptExec* vScriptExec, GrapaNames* pNameSpace, const char* pStr)
{
	...
	int len = snprintf((char*)sessionDebugStr.mBytes, sessionDebugStr.mLength, "[DEBUG-%llu] %s\n", (unsigned long long)mSessionId, pStr);
...
void GrapaScriptExecStateDebug::DebugPrint(GrapaScriptExec* vScriptExec, GrapaNames* pNameSpace, const char* component, const char* pStr, int level)
{
	...
	GrapaCHAR sessionDebugStr;
	sessionDebugStr.SetLength(256 + strlen(pStr));
	int len = snprintf((char*)sessionDebugStr.mBytes, sessionDebugStr.mLength, "[DEBUG-%llu-%s] %s\n", (unsigned long long)mSessionId, component, pStr);
	sessionDebugStr.SetLength(len);
	pNameSpace->GetResponse()->SendError(vScriptExec, pNameSpace, sessionDebugStr);
}
...
public:
	...
	virtual void Running()
	{
		...
		if (pInput->mToken == GrapaTokenType::LIST || pInput->mToken == GrapaTokenType::TUPLE || pInput->mToken == GrapaTokenType::GOBJ || pInput->mToken == GrapaTokenType::ERR || pInput->mToken == GrapaTokenType::XML || pInput->mToken == GrapaTokenType::EL || pInput->mToken == GrapaTokenType::TAG || pInput->mToken == GrapaTokenType::OP || pInput->mToken == GrapaTokenType::CODE)
		{
			GrapaRuleEvent cmd;

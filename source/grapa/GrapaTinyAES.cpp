...
void GrapaTinyAES::ClearKey()
{
	memset(vKey, 0, sizeof(AES_ctx));
	mSet = false;
}

void GrapaTinyAES::Init(const GrapaBYTE& pKey, const GrapaBYTE& pIV, int enc)
{
	GrapaBYTE key,iv;
	if (pKey.mLength == 32)
	{
...
void GrapaTinyAES::Encrypt(GrapaBYTE& pData)
{
...
void GrapaTinyAES::Decrypt(GrapaBYTE& pEnc)
{
...
void GrapaTinyAES::Encrypt(GrapaBYTE& pData, GrapaBYTE& pEnc)
{
	pEnc = pData;
	Encrypt(pEnc);
}

void GrapaTinyAES::Decrypt(GrapaBYTE& pEnc, GrapaBYTE& pData)
{
	pData = pEnc;
	Decrypt(pData);
}
...
/*
void GrapaTinyAES::EncryptSalt16(u8* value, u8* out, u8 saltCount)
{
	if (saltCount == 0)
	{
		memcpy(out, value, 16);
...
void GrapaTinyAES::DecryptSalt16(u8* value, u8* out, u8 saltCount)
{
	if (saltCount == 0) {
		memcpy(out, value, 16);
...
void GrapaTinyAES::EncryptSalt(GrapaBYTE& pData, u8 pSaltCount)
{
	GrapaBYTE enc;
...
void GrapaTinyAES::EncryptSalt(GrapaBYTE& pData, GrapaBYTE& pEnc, u8 pSaltCount)
{
	pEnc.SetLength(0);
...
void GrapaTinyAES::DecryptSalt(GrapaBYTE& pEnc)
{
	GrapaBYTE data;
...
void GrapaTinyAES::DecryptSalt(GrapaBYTE& pEnc, GrapaBYTE& pData)
{
	pData.SetLength(0);

...
void GrapaCHAR::AppendQuoted(GrapaCHAR& pStr, bool pIsOptional)
{
	...
	else if (pStr.mToken == GrapaTokenType::SYSID)
	{
		Append("$");
		Append(pStr);
	}
	...
	{
		if (pStr.mToken == GrapaTokenType::SYSSTR)
			Append("$");
...
void GrapaCHAR::RTrim2(GrapaRuleEvent* s)
{
	...
	else if (s->mValue.mToken == GrapaTokenType::LIST || s->mValue.mToken == GrapaTokenType::LIST)
	{
		bool found = true;
		while (mLength && found)
		{
...
void GrapaCHAR::LTrim2(GrapaRuleEvent* s)
{
	...
	else if (s->mValue.mToken == GrapaTokenType::LIST || s->mValue.mToken == GrapaTokenType::TUPLE)
	{
		bool found = true;
		u64 i = 0;

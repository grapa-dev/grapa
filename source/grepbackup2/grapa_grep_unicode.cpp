...
std::vector<MatchPosition> grep_unicode_impl(
    ...
    if (mode == ProcessingMode::BINARY_MODE) {
        ...
        unicode_rx.compile(); // Compile the pattern
            ...
            if (matched) {
                if (!match_only) {
                    ...
                    if (exact_match) {
                        ...
                        match_extract_rx.compile(); // Compile with the actual input to enable PCRE2
        ...
        if (matched) {
            if (!match_only) {
                ...
                if (exact_match) {
                    ...
                    UnicodeString match_extract_unicode_line(match_extract_line);
                    UnicodeRegex match_extract_rx(norm_pattern, ignore_case, diacritic_insensitive, norm);
                    match_extract_rx.compile(); // Compile the pattern
                    matches = match_extract_rx.find_all(match_extract_unicode_line);
                    // Map normalized match offsets back to original line
                    std::string cached_normalized_line;
                    if (!unicode_rx.is_ascii_mode()) {
...
ContextOptions parse_context_options(const std::string& options, std::string& filtered_options) {
    ...
    for (size_t i = 0; i < options.size(); ++i) {
        ...
        if (c == 'A' || c == 'B' || c == 'C') {
            ...
            i = j - 1;
        } else if (c == 'T' || c == 'w' || c == 'z' || c == 'L') {
            // Preserve these new options
            filtered_options += c;
        } else {
            filtered_options += c;
        }
...
std::vector<std::string> grep_extract_matches_unicode_impl_sequential(
    ...
    if (null_data && working_input.find('\0') != std::string::npos) {
        size_t start = 0, end;
        ...
        // For null-data mode, we need to process each record separately
        if (match_only) {
            std::vector<std::string> null_matches;
            for (const auto& record : lines) {
                if (!record.empty()) {
                    auto record_matches = grep_unicode_impl(record, effective_pattern, filtered_options, "", norm, mode);
                    for (const auto& match : record_matches) {
                        if (match.offset < record.size()) {
                            size_t end_offset = (match.offset + match.length < record.size()) ? 
                                match.offset + match.length : record.size();
                            std::string matched_text = record.substr(match.offset, end_offset - match.offset);
                            null_matches.push_back(matched_text);
                        }
                    }
                }
            }
    ...
    for (const auto& match : matches) {
        if (match.offset < working_input.size() || (match.length == 0 && match.offset == working_input.size())) {
            ...
            // Calculate column number if show_column is enabled
            if (show_column && match_only) {
                // Find which line this match belongs to
                size_t line_start = 0;
                size_t column = 0;
                for (size_t i = 0; i < line_offsets.size(); ++i) {
                    size_t line_end = (i + 1 < line_offsets.size()) ? line_offsets[i + 1] : working_input.size();
                    if (match.offset >= line_offsets[i] && match.offset < line_end) {
                        line_start = line_offsets[i];
                        column = match.offset - line_start;
                        break;
                    }
                }
                // Format as "column:match"
                matched_text = std::to_string(column) + ":" + matched_text;
            }
            
            // If color_output, wrap only the matched substring in ANSI color codes
            if (color_output && match_only) {
...
void UnicodeRegex::compile() {
    ...
    use_pcre_ = false;
#ifdef USE_PCRE
    // If pattern is non-ASCII, always use PCRE2
    if (!is_ascii_string(pattern_to_compile)) {
        use_pcre_ = true;
    }
#endif
    if (use_pcre_) {

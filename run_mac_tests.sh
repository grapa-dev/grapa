...
# This script finds and runs existing Unicode grep tests on Mac ARM64
...
# Function to find test files safely
find_test_files() {
    local pattern="$1"
    local files=()
...
for test_file in "${CPP_TEST_FILES[@]}"; do
    if [[ -f "$test_file" ]]; then
        run_cpp_test "$test_file" "$RESULTS_DIR/cpp_$(basename "$test_file" .cpp)_results.txt"
    else
...
for test_file in "${GRAPA_TEST_FILES[@]}"; do
    if [[ -f "$test_file" ]]; then
        run_grapa_test "$test_file" "$RESULTS_DIR/grapa_$(basename "$test_file" .grc)_results.txt"
    else
...
echo "   cd $RESULTS_DIR && ./run_specific_test.sh test_unicode_grep.cpp"
echo "   cd $RESULTS_DIR && ./run_specific_test.sh unicode_grep_comprehensive_tests.grc"
echo

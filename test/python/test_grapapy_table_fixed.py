#!/usr/bin/env python3
"""
Python Integration Test for Grapa Table Operations (Fixed, Refactored)
...
def test_table_operations_fixed():
    print("\n=== Testing Table Operations (Fixed) ===")
    xy = grapapy.grapa()
    try:
        ...
        print("Table operations test completed (fixed)")
        return True
    except Exception as e:
        print(f"Error in table operations (fixed): {e}")
        return False

def test_table_performance_fixed():
    print("\n=== Testing Table Performance (Fixed) ===")
    xy = grapapy.grapa()
    try:
        table = GrapaTableHelper(xy, "ROW")
        ...
        print("Table performance test completed (fixed)")
        return True
    except Exception as e:
        print(f"Error in table performance test (fixed): {e}")
        return False

def main():
    print("=== GRAPA TABLE OPERATIONS TEST (FIXED, REFACTORED) ===")
    print(f"Started at: {time.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"Working directory: {os.getcwd()}")
    passed = 0
    total = 2
    if test_table_operations_fixed():
        passed += 1
    if test_table_performance_fixed():
        passed += 1
    print(f"\n=== TEST SUMMARY ===\nTests passed: {passed}/{total}")
    if passed == total:
        print("\n🎉 All fixed table tests passed!\n")
        return 0
    else:
        print("\n❌ Some fixed table tests failed!\n")
        return 1

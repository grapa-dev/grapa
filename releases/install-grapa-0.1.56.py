...
class GrapaInstaller:
    ...
    def install_package(self, package_path):
        ...
        # Make executable
        grapa_exe = self.install_dir / "grapa-0.1.56" / "platforms" / self.detect_platform() / "grapa"
        if grapa_exe.exists():
            grapa_exe.chmod(0o755)
        
        print(f"✅ Grapa installed to {self.install_dir}")
        print(f"\nTo use Grapa, add to your PATH:")
        print(f"export PATH=\"{self.install_dir}/grapa-0.1.56/platforms/{self.detect_platform()}:$PATH\"")

...
public:
	...
	bool TestPrime(u32 safe, u32 confidence) const;
	bool AKS() const;
public:
	bool FermatLittleTest(int confidence) const;
	bool RabinMillerTest(int confidence) const;
	bool SolovayStrassenTest(int confidence) const;
	bool LucasStrongTest() const;
	bool LucasStrongTestHelper(const GrapaInt& thisVal) const;
	bool DivisorTest(u32 safe) const;
	bool ConfidenceTest(int confidence) const;
	bool StrongTest() const;
	bool isProbablePrime() const;
	static s32 Jacobi(const GrapaInt& a, const GrapaInt& b);
	static s32 Jacobi(s64 a, const GrapaInt& b);
	GrapaInt genCoPrime(u32 bits)const;
	void GenPrimes16();
